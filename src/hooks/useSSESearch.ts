...
export default function useSSESearch({searchQuery, sourcesSelected, algoSelected, dialogHandle, initialResultFilename}: Props) {
    ...
    function setResultFilename(filename: SearchFilenameType){
        setResultFilenamePrivate(filename)
        globalState.searchRef.current.resultFilename = filename
    }
