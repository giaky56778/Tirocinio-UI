...
import {
    ...
    HighlightLine,
    resolveLineByWordId
} from "@/components/reducer/wordHighlightReducer.ts";
...
export type SelectionOpType={
    ...
    copyHighlight: (highlightId: string | null, highlightBounds: Record<string, HighlightBound>, side: Side) => Promise<void>
...
export function selectionReducer(state: SelectionState, action: SelectionAction): SelectionState {
    switch (action.type) {
        case "START_SELECTION": {
            ...
            const newRange: SelectionRange = { start: wordId, end: wordId, side }
...
function updateSelectionRecord(
    prevRecord: Record<number, HighlightLine>,
    ...
    if (newStartLine === -1 || newEndLine === -1) return prevRecord;
    ...
    for (let i = minStart; i <= maxStart; i++) affectedLines.add(i);

    const minEnd = Math.min(oldEndLine, newEndLine);
    const maxEnd = Math.max(oldEndLine, newEndLine);
    for (let i = minEnd; i <= maxEnd; i++) affectedLines.add(i);
    ...
    for (const line of affectedLines) {
        ...
        const currentMarker = nextRecord[line];
        ...
        if (!newMarker) {
            delete nextRecord[line];
            changed = true;
        } else {
            nextRecord[line] = newMarker;
            changed = true;
        }
    }

    return changed ? nextRecord : prevRecord;
}