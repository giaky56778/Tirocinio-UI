import {DragType} from "@/hooks/useHighlightUI.ts";
import {TextIndexSchema} from "@/utils/JSONSchema.ts";
import {HighlightColor, HighlightSingle} from "@/components/editor/doubleEditor.tsx";
...
function upsertLineMarker(id: number, lineInfo: HighlightLine, startEndPositionInit:Record<number,HighlightLine[]>) {
    ...
    if (boundFind!==undefined) {
        if (lineInfo.startWord !== undefined) boundFind.startWord = lineInfo.startWord
        if (lineInfo.endWord !== undefined) boundFind.endWord = lineInfo.endWord
        return
    }
    row.push({...lineInfo})
}
...
export function buildHighlightMarkerForLine(
    ...
    const start = newBound.lineStart //Math.min(newBound.lineStart, newBound.lineEnd)
    const end = newBound.lineEnd     //Math.max(newBound.lineStart, newBound.lineEnd)
            ...
            if( newEnd>= lineIndex &&
                ...
                return {
                    ...
                    error:{
                        ...
                        message: 'Right handle cannot be move in that position',
                        maxId:m.startWord-1,
                        handle: 'right',
                ...
                return {
                    ...
                    error:{
                        ...
                        message:'Left handle cannot be move in that position',
                        maxId:m.endWord+1,
                        handle:'left',
                        highlightId:drag.highlightId
                    }
