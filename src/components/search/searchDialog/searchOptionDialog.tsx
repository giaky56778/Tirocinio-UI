...
import AlgorithmOptionItem from "@/components/search/searchDialog/algorithmButton.tsx";
...
import {CheckIcon, InfoIcon, MinusIcon, TriangleExclamationIcon} from "@/components/icons";
...
export default function SearchOptionDialog({settings, tooltip, algoSelected,setAlgoSelected}: SearchAlertProps) {
    ...
    const [value, setValue] = React.useState<string[]>(() => algoSelected.slice())
    ...
    const toggleAlgorithm = (key: string) => {
        setValue(v => v.includes(key) ? v.filter(x => x !== key) : [...v, key]);
    };
    ...
    return (
        <Tooltip.Provider>
            <Dialog.Root disablePointerDismissal={algoError}>
                <Dialog.Trigger
                    className={'mx-auto h-8 p-3 text-sm border w-full bg-orange-500 hover:bg-orange-700 cursor-pointer transition-colors text-white rounded shadow-lg font-medium flex items-center justify-center'}
                ...
                <Dialog.Portal style={{userSelect:'none'}}>
                    ...
                    <Dialog.Popup className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex w-187.5 max-w-[calc(100vw-3rem)] flex-col gap-4 bg-white border p-6">
                        <DialogCloseCostume/>
                        ...
                        <div
                            ...
                            tabIndex={0}
                            onClick={() => {
                                if (value.length === hybridAlgorithm.length) setValue([])
                                else setValue(hybridAlgorithm.slice())
                            }}
                            onKeyDown={(e) => {
