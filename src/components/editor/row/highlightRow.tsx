...
import {ChapterTitleItem, TextLineItem, TitleTextItem} from "@/utils/JSONSchema.ts";
import {composeTitle, createGroup, WordGroup} from "@/utils/util.ts";
import {CostumeContextType} from "@/components/editor/contextMenuCostume.tsx";
import GroupRenderer from "./groupRender.tsx";
import {SearchHighlight, SelectionRange} from "@/components/reducer/selectionReducer.ts";
import {TEXT_FONT_CLASS, Side} from "@/utils/globalType.ts";
...
function HighlightRow({side, text, lineIndex, lineHighlights, highlightBounds, highlightState, colors, contextMenu, previewCardHandler, blinkHighlightId, toolBarVisible, searchHighlight, lineSelection, onClearSelection,}: Props) {
    ...
    return (
        ...
        >
            ...
            <span className={`col-start-2 min-w-0 ${side === 'historical' ? TEXT_FONT_CLASS : ''}`}>
...
function areHighlightRowPropsEqual(prevProps: Props, nextProps: Props) {
...
export default memo(HighlightRow, areHighlightRowPropsEqual)
