...
import {colorMap, Side, TEXT_FONT_CLASS} from "@/utils/globalType.ts";
...
export default function RenderPreviewCardText({text, color,side}: RenderPreviewCardTextProps) {
    const colorClass = color != null && colorMap[color] != null ? colorMap[color].class : undefined
    return (
                        ...
                        <React.Fragment key={index}>
                            ...
                            <span className="col-start-2 min-w-0">
                                {line.group.map(item => {
                                    ...
                                    return (
                                        <span className={`${side === 'historical' ? TEXT_FONT_CLASS : ''}`}>
