...
import {useUploadText} from "@/hooks/useUploadText.ts";
...
function UploadTextForm({onTextChange, dialogHandle}: Props){
    ...
    const {
        ...
        text, setText,
        inputRef, hasName,
        applyFile, removeFile, handleSubmit, uploadMutation
        ...
        >
            ...
            >
                <Tabs.List className="relative flex gap-6 border-b border-neutral-200">
                    ...
                    <Tabs.Indicator
                        className="absolute -bottom-px left-0 h-0.5 w-(--active-tab-width) translate-x-(--active-tab-left) bg-blue-600 transition-all duration-200 ease-out"/>
                </Tabs.List>
                ...
                >
                    <label
                        ...
                        className={`
                            ...
                            ${isDragging ? "border-blue-500" : "border-neutral-300 hover:border-neutral-400"}
                        `}
                    >
                        <span className="text-sm text-neutral-600">
                            Trascina un file o <span className="text-blue-600 underline underline-offset-2">selezionalo</span>
                        </span>
            ...
            <Button
                type="submit"
                disabled={!(hasName && (mode === "file" ? !!file && !!fileFormat : text.trim().length > 0)) || uploadMutation.isPending}
                className="w-full rounded-md bg-blue-600 py-2.5 text-sm font-medium text-white transition-colors hover:enabled:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-30"
            >
